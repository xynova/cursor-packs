...
- [ ] Typology slice stages use domain language (`typology_slice_grouping` / `_meaning` / `_catalog`); new primary uses of `typology_cluster` / `typology_refine` / `typology_objective_grounding` without alias context → clarity finding (see `.cursor/rules/typology-slice-domain.mdc`)
