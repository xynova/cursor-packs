...
- **AI cadence (evaluation):** Human-output prose jobs MUST append strop's `ai_cadence` role (`dspy.AppendAICadenceEvaluator` / `evaluation/aicadence`) on a cheap model via `roleProviders`, parallel to the interceptor MUST for retries. Skip vernacular-aphorism and structural jobs. See `strop-pipeline-pattern` §3.
